...
Private Sub Worksheet_AutoFilter_ShowAll(ws As Worksheet)
    ...
    '
    'External Functions used (Public Functions)
    '1.) GetLastUsedRow
    '2.) GetLastUsedColumn
    '3.) GetRangeFromCells
    ...
    wsRows = GetLastUsedRow(ws:=ws)
    wsCols = GetLastUsedCol(ws:=ws)
...
Public Function GetLastUsedCol(ws As Worksheet) As Long
    ...
    GetLastUsedCol = ws.Range("A1").SpecialCells(xlCellTypeLastCell).Column
